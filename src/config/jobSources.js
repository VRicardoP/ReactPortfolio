...
export const JOB_SOURCES = [
  ...
  },
  {
    key: 'thehub',
    color: '#1de9b6',
    urlPath: '/api/v1/thehub-jobs/recent',
    skillsField: 'tags',
    alwaysRemote: true,
    normalize: (job) => ({
      id: job.id ? `thehub-${job.id}` : `thehub-${Date.now()}-${Math.random().toString(36).slice(2)}`,
      title: job.title || '',
      company: job.company || '',
      location: job.location || 'Remote',
      date: job.date || '',
  ...
  },
  {
    key: 'jobgether',
    color: '#7c4dff',
    urlPath: '/api/v1/jobgether-jobs/recent',
    skillsField: 'tags',
    alwaysRemote: true,
    normalize: (job) => ({
      id: job.id ? `jobgether-${job.id}` : `jobgether-${Date.now()}-${Math.random().toString(36).slice(2)}`,
      title: job.title || '',
      company: job.company || '',
      location: job.location || 'Remote',
      date: job.date || '',
  ...
  },
  {
    key: 'nav',
    color: '#0d47a1',
    urlPath: '/api/v1/nav-jobs/recent',
    skillsField: 'tags',
    alwaysRemote: false,
    normalize: (job) => ({
      id: job.id ? `nav-${job.id}` : `nav-${Date.now()}-${Math.random().toString(36).slice(2)}`,
      title: job.title || '',
      company: job.company || '',
      location: job.location || 'Norway',
      date: job.date || '',
      url: job.url || '',
      tags: job.tags || [],
      remote: job.remote === true,
  ...
  },
  {
    key: 'irishjobs',
    color: '#2e7d32',
    urlPath: '/api/v1/irishjobs-jobs/recent',
    skillsField: 'tags',
    alwaysRemote: false,
    normalize: (job) => ({
      id: job.id ? `irishjobs-${job.id}` : `irishjobs-${Date.now()}-${Math.random().toString(36).slice(2)}`,
      title: job.title || '',
      company: job.company || '',
